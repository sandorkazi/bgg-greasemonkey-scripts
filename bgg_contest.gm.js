...
function parseAnswer(answer) {
    return {
        'radio': answer,
        'text': answer.nextElementSibling.textContent.trim(),
    }
}

function parseQuestion(box) {
    let question = box.getElementsByTagName("h2")[0].childNodes[2].textContent;
...
function parseQuestions(response) {
    ...
    let boxes = Array.from(page.querySelectorAll('.question-block'));
    return {
